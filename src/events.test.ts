...
const fixedConfig: NotifySoundConfig = {
  sound: true,
  sounds: { default: "d.wav", complete: "c.wav", question: "q.wav", error: "e.wav" },
  events: {
...
vi.mock("./config.js", () => ({
  ...
  resolveSoundPath: (cfg: NotifySoundConfig, sound?: string) => {
    if (sound && sound in cfg.sounds) return cfg.sounds[sound as keyof NotifySoundConfig["sounds"]] || cfg.sounds.default || null;
...
function handlerFor(pi: MockPi, channel: string): (...args: unknown[]) => void {
  ...
  const call2 = call ?? pi.events.on.mock.calls.find((c: string[]) => c[0] === channel);
...
describe("wireEvents", () => {
  it("registers agent_settled via pi.on and prompt events via pi.events.on", () => {
    ...
    expect(pi.events.on).toHaveBeenCalledWith("rpiv:ask-user:prompt", expect.any(Function));
    expect(pi.events.on).toHaveBeenCalledWith("permissions:ui_prompt", expect.any(Function));
  });
