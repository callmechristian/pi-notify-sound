...
import { DEFAULT_CONFIG, ensureConfigFile, loadConfig, resolveSoundPath } from "./config.js";
...
function withSound(patch: Partial<NotifySoundConfig["sounds"]>): NotifySoundConfig {
...
describe("loadConfig", () => {
  ...
  it("merges partial config over defaults", () => {
    const p = tempConfig(JSON.stringify({ sound: false, sounds: { complete: "C:\\x.wav" } }));
    try {
      const cfg = loadConfig(p);
...
describe("resolveSoundPath", () => {
  it("resolves a sound key and falls back to default for empty values", () => {
    const cfg = withSound({ default: "d.wav", complete: "c.wav", question: "" });
  ...
  it("passes through absolute paths untouched", () => {
    expect(resolveSoundPath(DEFAULT_CONFIG, "E:\\a\\b.wav")).toBe("E:\\a\\b.wav");
  });
